...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Dispatch Python frontend UI debug export to a module-local script.")
    ...
    if args.list_modules:
        for module_name in sorted(discover_module_scripts("ui_debug_runner")):
            print(module_name)
        return 0

    _, script_path = resolve_module_script("ui_debug_runner", args.module)
